...
1. Change layers
2. Second reclassify option-gradient
3. Sum of binary
4. summary tables for summed across projections
5. Add datatype INT1U to each binary write raster-but only 0-1 binary
...
if (scenario=="MainlandCA"){
  ...
  hist<-raster(paste0("G:/data/tools/MaxEnt/Output/", scenario, "/", h, "/", filename,"/Quercus_tomentella_layers_avg.asc"))
...
writeRaster(histrc, filename = paste0("G:/data/tools/MaxEnt/Output_tiff/", scenario, "/binary/", h, ".tif"), format="GTiff", overwrite=TRUE)
...
percsuit<-as.data.frame(percsuitmat, row.names = FALSE)

projtable<-cbind(projTestAUC, highsuit, meandiff, gain, loss, percsuit, pcsuit)
